...
一组 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 社区插件，面向本地嵌入式与工业控制台架：传统调试界面、上位机监控和当前 Session 的「台架模式」Agent 围绕同一份现场状态协同工作。
...
| 插件 | 职责 | 仓库 |
|---|---|---|
| [dsh-vision-bench](./dsh-vision-bench/) | 会话区调试 / 上位机，Keil 编译、Modbus 读写（FC01–06/15/16）、共享任务与时间线，设置页绑定本机 Python / Keil UV4 / OpenOCD，并安装「台架模式」Agent 预设 | https://github.com/xingyingyuzhui/dsh-vision-bench |
...
装完重启 `dsh web`。新会话选 **台架模式** 后，Agent 用 `vision_bench` 按需查询和编译/读点/写点。
